...
// itself becomes the exit code: 0 auto-merge, 3 parked, 1 usage error, 2 unreadable board or config.
...
} else if (mode === "--queue-audit") {
  // Integrity of the control plane itself. Reports repairs; never performs one.
  // The repair for a colliding row is a judgment call about which row is
  // canonical, and that belongs to the operator, not to a parser.
  const queuePath = rest[0] ?? DEFAULT_QUEUE;
  const lanesPath = rest[1] ?? DEFAULT_LANES;
  let findings;
  try {
    findings = auditQueue(read(queuePath), readLaneFile(lanesPath));
  } catch (e) {
    die(`Cannot audit: ${e.message}`, 2);
  }
  ...
  if (jsonMode) {
    console.log(
      JSON.stringify({
        ...
        command: "queue-audit",
        args: { queue: queuePath, lanes: lanesPath },
        data: { findings, counts: { total: findings.length, blocker: blockers.length } },
        problems: [],
        warnings: [],
      })
    );
  } else if (!findings.length) {
    console.log("## Queue audit\n\nCLEAN. The queue and the lane board agree.");
  } else {
    console.log(`## Queue audit\n\n${findings.length} finding(s), ${blockers.length} blocker(s).\n`);
    for (const f of findings) {
      console.log(`- [${f.severity}] ${f.check} (row ${f.row})`);
      console.log(`  ${f.message}`);
      console.log(`  repair: ${f.repair}`);
    }
    console.log(
      "\nThese are reported, not applied. Each repair is a judgment call about which row is canonical."
    );
  }
  // --ci makes a blocker a non-zero exit so a pipeline can refuse to launch on a
  // corrupt board. Without it the audit is a report and exits 0, park-style.
  if (ciMode) process.exit(blockers.length ? 3 : 0);
} else {
  die(
    "Usage: lane-report.mjs --eligible|--status|--metrics|--landed|--cap|--gate|--queue-audit [files] [flags]",
    1
  );
}
