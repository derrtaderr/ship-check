...
// ---------------------------------------------------------------------------
// auditQueue — integrity of the control plane itself.
//
// The queue and the lane board are the two mutable surfaces this whole tool
// reads. They drift, and every drift silently corrupts what the launcher offers
// next: a row ships and stays eligible so the launcher keeps re-offering built
// work; a lane runs with no row so the board and the queue disagree about what
// exists; a second row claims an ID an earlier row already owns, so a card, a
// commit message and a conversation that all say "row 34" mean three different
// things. Each of these has actually happened on a live board.
//
// It REPORTS repairs and never performs them. A control plane that silently
// rewrites itself is exactly the failure this tool exists to refuse: the repair
// is a judgment call about which of two colliding rows is canonical, and that
// belongs to the operator, not to a parser.
//
// Returns an array of findings, each { check, severity, row, message, repair }.
// An empty array means the two surfaces agree.
// ---------------------------------------------------------------------------
export function auditQueue(queueMd, lanesMd) {
  if (!laneTableFound(lanesMd))
    throw new Error(
      'No lane table found: the header row starting "| lane |" was not found. ' +
        "Refusing rather than reading an unreadable board as an all-clear.",
    );

  const rows = parseQueueRows(queueMd);
  const lanes = parseLanes(lanesMd);
  const findings = [];
  const add = (check, severity, row, message, repair) =>
    findings.push({ check, severity, row, message, repair });

  // 1. Duplicate row IDs. Row numbers are permanent identifiers that cards,
  //    commits and conversations cite, so one number naming two different
  //    builds makes every one of those references ambiguous. The repair is
  //    never "renumber the old one" — the older row keeps the ID it earned and
  //    the newcomer moves above the high-water mark, leaving an alias line.
  const byNumber = new Map();
  for (const r of rows) {
    if (!byNumber.has(r.number)) byNumber.set(r.number, []);
    byNumber.get(r.number).push(r);
  }
  const highWater = rows.reduce((max, r) => Math.max(max, r.number), 0);
  let nextFree = highWater + 1;
  for (const [number, group] of byNumber) {
    if (group.length < 2) continue;
    const titles = group.map((g) => `"${g.title}"`).join(" and ");
    add(
      "duplicate-row-id",
      "blocker",
      number,
      `Row ${number} names ${group.length} different builds: ${titles}. ` +
        "Every card, commit and conversation citing this number is ambiguous.",
      `Keep the earlier row at ${number}; move the later one(s) to ${nextFree}+ ` +
        `and leave a one-line alias ("formerly duplicate row ${number}") so old references resolve.`,
    );
    nextFree += group.length - 1;
  }

  // 2. Shipped work still offered as launchable. The launcher ranks on
  //    eligible=yes, so a row whose lane already shipped keeps being offered as
  //    unbuilt until someone notices by hand.
  const shippedRowNumbers = new Set(
    lanes.filter((l) => l.status === "shipped" && l.row != null).map((l) => l.row),
  );
  for (const r of rows) {
    if (r.eligible && shippedRowNumbers.has(r.number))
      add(
        "shipped-still-eligible",
        "blocker",
        r.number,
        `Row ${r.number} ("${r.title}") is eligible=yes, but a lane for row ${r.number} is recorded shipped. ` +
          "The launcher will keep offering work that is already built.",
        `Set eligible=no on row ${r.number} (and record what remains, if anything).`,
      );
  }

  // 3. A lane on the board with no row in the queue. The board says work is in
  //    flight; the queue does not know it exists.
  const rowNumbers = new Set(rows.map((r) => r.number));
  const OPEN = new Set(["running", "pr-open", "parked", "stalled"]);
  for (const l of lanes) {
    if (!OPEN.has(l.status)) continue;
    if (l.row == null || rowNumbers.has(l.row)) continue;
    add(
      "lane-missing-row",
      "important",
      l.row,
      `Lane ${l.lane ?? "?"} (${l.repo ?? "unknown repo"}, ${l.status}) cites row ${l.row}, ` +
        "which does not exist in the queue. The board and the queue disagree about what is in flight.",
      `Add the row to the queue, or correct the lane's row cell to the row it actually built.`,
    );
  }

  // 4. An eligible row the launcher cannot actually dispatch. Repo independence
  //    and unit sizing are what make a pick safe; a row missing either is a
  //    promise the launcher cannot keep.
  for (const r of rows) {
    if (!r.eligible) continue; // an unscoped candidate is not a defect
    if (!r.repo)
      add(
        "eligible-missing-repo",
        "important",
        r.number,
        `Row ${r.number} ("${r.title}") is eligible=yes but declares no repo, so it cannot be dispatched.`,
        `Add repo=\`<name>\` to row ${r.number}'s Lane line, or set eligible=no until it is scoped.`,
      );
    if (r.unit == null)
      add(
        "eligible-missing-unit",
        "important",
        r.number,
        `Row ${r.number} ("${r.title}") is eligible=yes but declares no unit, so its size is unknown.`,
        `Add unit=<n> to row ${r.number}'s Lane line, or set eligible=no until it is sized.`,
      );
  }

  // 5. Two eligible rows on one repo. Launching both means a second branch off
  //    that repo while the first is open, which orphans the first into conflict.
  const byRepo = new Map();
  for (const r of rows) {
    if (!r.eligible || !r.repo) continue;
  ...
  for (const [repo, group] of byRepo) {
    if (group.length < 2) continue;
    add(
      "duplicate-active-repo",
      "important",
      group[0].number,
      `Rows ${group.map((g) => g.number).join(", ")} are all eligible on repo \`${repo}\`. ` +
        "Launching two of them opens a second branch off one repo and orphans the first PR.",
      `Leave one eligible=yes on \`${repo}\` and hold the rest at eligible=no until it ships.`,
    );
  }
